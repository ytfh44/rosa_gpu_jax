...
from rosa_gpu_jax import lookup_full_l_dp, make_raw_causal_aux, make_rosa_causal_aux
from rosa_gpu_jax.reference import brute_force_lookup, rosa_batch_reference_tau


def test_dp_smoke_vs_bruteforce():
...
def test_dp_random_vs_bruteforce():
    ...
    tau, match_len = lookup_full_l_dp(Q, K, cap_end, successor, Lmax=Lmax)
    tau_ref, len_ref = brute_force_lookup(
...
def test_dp_rosa_counterexample_no_backtracking():
    """DP must NOT backtrack to an older match when the rightmost has no valid successor."""
    Z = np.array([[[0, 1, 0, 0]]], dtype=np.int64)
    cap, succ, tau_cap = make_rosa_causal_aux(jnp.asarray(Z))

    tau, match_len = lookup_full_l_dp(
...
def test_dp_enumerates_all_binary_len_6():
...
def test_dp_custom_cap_and_successor():
...
def test_dp_lmax_larger_than_t_rejected():
    Q = jnp.array([[[0, 1, 2]]], dtype=jnp.int64)
    K = Q
    cap_end, successor = make_raw_causal_aux(1, 1, 3)
    with np.testing.assert_raises(ValueError):
        lookup_full_l_dp(Q, K, cap_end, successor, Lmax=4)
